...
Base Date : 2010-01-04  |  Base Level : 100.0 (both ER and TR)
    (Bloomberg 방법론상의 공식 Base Date. 본 프로젝트의 실제 백테스트·성과 분석은
     보유 선물 데이터 구간에 맞춰 2016-01-04 = 100.0 으로 리베이스한 값을 사용합니다 — README §2-5 참조.)
...
# ============================================================
# 파트 15: 월별 비교 계산 (정적 vs DRA 순수 롤일드·체결금액)
# IRY 계산 기준 통일: 두 전략 모두 결정일(BD6) 결제가격 기준
# 공식: IRY = (P_roll_out - P_roll_in) / (P_roll_in × Δmonths)
# ============================================================
...
def _compute_monthly_comparison(
    ...
    """월별 정적(ETN BM) vs 동적(DRA) 순수 롤일드(Pure Roll Yield) 비교.

    IRY 계산 기준 통일:
    - 정적: roll_out=당월 최근월물(M1), roll_in=차근월물(M2)
    - 동적: roll_out=dra_sched lead_ticker, roll_in=dra_sched next_ticker
      roll_required=False이면 dynamic_iry=0.0 (롤 없으므로 비용 없음)
    두 기준 모두 결정일(BD6, det_date) 결제가격 사용.
    """
    ...
    for yr, mo in months:
        ...
        # optimum_set: 정보 제공용 (monthly_comparison.csv 유지)
        ranked = _calculate_iry(contracts)
        optimum_set = [s for s, _ in ranked[:3]]

        # static: M1 → M2 (당월 최근월물 → 차근월물)
        ...
        def _pure_iry(roll_out_sym: str, roll_in_sym: str) -> float:
            """두 티커 간 순수 롤일드. 가격/만기 없으면 NaN."""
            if roll_out_sym not in sym_map or roll_in_sym not in sym_map:
                return float('nan')
            p_out, e_out = sym_map[roll_out_sym]
            p_in,  e_in  = sym_map[roll_in_sym]
            if p_in <= 0:
                return float('nan')
            interval = (e_in.year - e_out.year) * 12 + (e_in.month - e_out.month)
            interval = max(interval, 1)
            return (p_out - p_in) / (p_in * interval)

        # --- 순수 롤일드: 정적 ---
        static_pure_iry = _pure_iry(static_roll_out, static_roll_in)

        # --- 순수 롤일드: 동적 ---
        # roll_required=False → 롤 없음 → 비용 0.0 (NaN 아님)
        if not roll_required:
            dynamic_pure_iry = 0.0
        else:
            dynamic_pure_iry = _pure_iry(dynamic_roll_out, dynamic_roll_in)

        pure_iry_saving = (
            dynamic_pure_iry - static_pure_iry
            if not (np.isnan(dynamic_pure_iry) or np.isnan(static_pure_iry))
            else float('nan')
        )
        ...
        results.append({
            ...
            # 통일된 순수 롤일드 — 기존 키 유지(monthly_comparison.csv 호환)
            "static_iry":                static_pure_iry,
            "dynamic_iry":               dynamic_pure_iry,
            "iry_saving":                pure_iry_saving,
            # pure_roll_yield.csv 전용 추가 키
            "static_pure_iry":           static_pure_iry,
            "dynamic_pure_iry":          dynamic_pure_iry,
            "pure_iry_saving":           pure_iry_saving,
...
# ============================================================
# 파트 15-B: 유효 롤 비용(Effective Roll Cost) 계산
# ============================================================

def calculate_effective_roll_cost(
    bdays: pd.DatetimeIndex,
    prices_wide: pd.DataFrame,
    expiration_map: dict,
    static_result: pd.DataFrame,
    dra_result: pd.DataFrame,
) -> Tuple[pd.DataFrame, pd.DataFrame]:
    """
    일별 / 월별 유효 롤 비용(Effective Roll Cost) 계산.

    공식:
        유효 롤 비용_t = ER_t / ER_{t-1}  -  M1_t / M1_{t-1}

    M1: 각 날짜 기준 만기가 가장 가까운 선물 계약 (expiration_map 기준).
    M1 만기 교체일(티커 변경일)은 M1 수익률 0.0 처리.

    연환산:
        누적 = ∏(1 + 일별 유효 롤 비용) - 1
        연환산 = (1 + 누적)^(252 / 전체 영업일 수) - 1

    Parameters
    ----------
    bdays         : CME 영업일 인덱스
    prices_wide   : 선물 결제가격 DataFrame (wide, 인덱스=날짜, 컬럼=티커)
    expiration_map: 티커 → 만기일 dict
    static_result : run_index_calculation() 반환 DataFrame (er_index 컬럼 필요)
    dra_result    : run_dra_index_calculation() 반환 DataFrame (er_index 컬럼 필요)

    Returns
    -------
    (daily_df, monthly_df)
      daily_df   : date 인덱스, 일별 유효 롤 비용 컬럼
      monthly_df : 월별 평균 집계 + 마지막 행에 전체 기간 연환산 값
    """
    dates = list(bdays)
    ...
    m1_prices_map: dict = {}
    for date in dates:
        contracts = _get_contracts_for_date(prices_wide, expiration_map, date, n=1)
        if contracts:
            m1_tickers[date]    = contracts[0][0]
            m1_prices_map[date] = contracts[0][1]
        else:
            m1_tickers[date]    = None
            m1_prices_map[date] = np.nan

    # ER 시리즈를 bdays 기준으로 정렬
    static_er = static_result["er_index"].reindex(bdays)
    dra_er    = dra_result["er_index"].reindex(bdays)
    ...
    for i, date in enumerate(dates):
        ...
        # M1 수익률 비율 계산
        m1_curr = m1_tickers.get(date)
        m1_prev = m1_tickers.get(prev)

        if m1_curr is None or m1_prev is None or m1_curr != m1_prev:
            # 만기 교체일: 0.0 처리 (ratio = 1.0)
            m1_ratio = 1.0
            m1_ret   = 0.0
        else:
            p_curr = m1_prices_map.get(date, np.nan)
            p_prev = m1_prices_map.get(prev, np.nan)
            if np.isnan(p_curr) or np.isnan(p_prev) or p_prev == 0.0:
                m1_ratio = 1.0
                m1_ret   = 0.0
            else:
                m1_ratio = p_curr / p_prev
                m1_ret   = m1_ratio - 1.0

        # Static ER 유효 롤 비용
        s_er_t   = static_er.get(date,  np.nan)
        s_er_tm1 = static_er.get(prev,  np.nan)
        if pd.isna(s_er_t) or pd.isna(s_er_tm1) or s_er_tm1 == 0.0:
            s_er_ret = np.nan
            s_erc    = np.nan
        else:
            s_ratio  = s_er_t / s_er_tm1
            s_er_ret = s_ratio - 1.0
            s_erc    = s_ratio - m1_ratio

        # DRA ER 유효 롤 비용
        d_er_t   = dra_er.get(date,  np.nan)
        d_er_tm1 = dra_er.get(prev,  np.nan)
        if pd.isna(d_er_t) or pd.isna(d_er_tm1) or d_er_tm1 == 0.0:
            d_er_ret = np.nan
            d_erc    = np.nan
        else:
            d_ratio  = d_er_t / d_er_tm1
            d_er_ret = d_ratio - 1.0
            d_erc    = d_ratio - m1_ratio

        daily_rows.append({
            "date":                               date,
            "m1_ticker":                          m1_curr,
            "m1_price":                           m1_prices_map.get(date, np.nan),
            "m1_daily_return":                    m1_ret,
            "static_er_daily_return":             s_er_ret,
            "static_effective_roll_cost_daily":   s_erc,
            "dynamic_er_daily_return":            d_er_ret,
            "dynamic_effective_roll_cost_daily":  d_erc,
        })
    ...
    # 월별 집계
    monthly_rows = []
    for (yr, mo), grp in daily_df.groupby(
        [daily_df.index.year, daily_df.index.month]
    ):
        month_dra = dra_result[
            (dra_result.index.year == yr) & (dra_result.index.month == mo)
        ]
        roll_req = (
            bool(month_dra["roll_required"].iloc[0])
            if not month_dra.empty and "roll_required" in month_dra.columns
            else False
        )
        monthly_rows.append({
            "date":   grp.index[0],
            "year":   yr,
            "month":  mo,
            "static_effective_roll_cost_monthly_avg":
                grp["static_effective_roll_cost_daily"].mean(),
            "dynamic_effective_roll_cost_monthly_avg":
                grp["dynamic_effective_roll_cost_daily"].mean(),
            "roll_required": roll_req,
        })
    ...
    def _annualize(series: pd.Series) -> float:
        ...
        if len(valid) == 0:
            return np.nan
        cumulative = (1.0 + valid).prod() - 1.0
        return (1.0 + cumulative) ** (252.0 / len(valid)) - 1.0

    s_ann = _annualize(daily_df["static_effective_roll_cost_daily"])
    d_ann = _annualize(daily_df["dynamic_effective_roll_cost_daily"])

    ann_row = pd.DataFrame([{
        "date":   pd.NaT,
        "year":   "TOTAL",
        "month":  "annualized",
        "static_effective_roll_cost_monthly_avg":  s_ann,
        "dynamic_effective_roll_cost_monthly_avg": d_ann,
        "roll_required": np.nan,
    }])
    monthly_df = pd.concat([monthly_df, ann_row], ignore_index=True)

    return daily_df, monthly_df


# ============================================================
# 파트 15-C: 순수 롤 스프레드 ($/배럴) 계산
# 공식: 일별 롤 스프레드 = P_roll_out - P_roll_in  ($/배럴)
#        월별 롤 스프레드 = BD6~BD10 일별 스프레드의 평균
# ============================================================
...
def calculate_roll_cost(
    ...
    """
    순수 롤 스프레드($/배럴)를 정적/동적 두 가지 방식으로 계산.

    공식:
        일별 롤 스프레드 = P_roll_out - P_roll_in  ($/배럴)
        월별 롤 스프레드 = BD6~BD10 일별 스프레드의 평균
          - 가격 없는 날(NaN)은 제외하고 평균
          - 롤 없는 달(roll_required=False) → 0.0

    시장상황 판단은 BD6 스냅샷 기준 (M1 vs M2).

    Parameters
    ----------
    result       : run_dra_index_calculation() 반환 DataFrame
    prices_wide  : 선물 결제가격 DataFrame (wide, 인덱스=날짜, 컬럼=티커)
    dra_sched    : build_dra_contract_schedule() 반환 DataFrame
    bdays        : CME 영업일 DatetimeIndex
    prefix       : 선물 티커 접두사 (기본 'CL')

    Returns
    -------
    (monthly_df, summary_df)
      monthly_df : 월별 상세 — roll_cost_monthly.csv 용
      summary_df : 전체 기간 요약 (행 기반) — roll_cost_summary.csv 용
    """
    months = sorted(set((d.year, d.month) for d in bdays))

    def _get_price(ticker: str, date: pd.Timestamp) -> float:
        if ticker in prices_wide.columns and date in prices_wide.index:
            v = prices_wide.at[date, ticker]
            return float(v) if not pd.isna(v) else float("nan")
        return float("nan")

    def _avg_spread(roll_out_ticker: str, roll_in_ticker: str, roll_days: list) -> float:
        """BD6~BD10 각 날의 (P_out - P_in) 평균. 유효 가격 없으면 NaN."""
        spreads = []
        for d in roll_days:
            p_out = _get_price(roll_out_ticker, d)
            p_in  = _get_price(roll_in_ticker,  d)
            if not (pd.isna(p_out) or pd.isna(p_in)):
                spreads.append(p_out - p_in)
    ...
    for yr, mo in months:
        ...
        det_date  = month_bdays[ROLL_START_BD - 1]          # BD6 (시장상황 판단용)
        roll_days = month_bdays[ROLL_START_BD - 1 : ROLL_END_BD]  # BD6~BD10
        ...
        def _c_year(code: str) -> int:
            cm = FUTURES_CODE_TO_MONTH[code]
            return yr + 1 if cm <= mo else yr

        static_roll_out = f"{prefix}{lead_code}{str(_c_year(lead_code))[-2:]}"
        static_roll_in  = f"{prefix}{next_code}{str(_c_year(next_code))[-2:]}"

        static_roll_spread = _avg_spread(static_roll_out, static_roll_in, roll_days)

        # ── 시장 상황 (BD6 스냅샷: 정적 M1 vs M2) ──────────────
        p_out_bd6 = _get_price(static_roll_out, det_date)
        p_in_bd6  = _get_price(static_roll_in,  det_date)
        if pd.isna(p_out_bd6) or pd.isna(p_in_bd6):
            market_condition = "Unknown"
        elif p_out_bd6 > p_in_bd6:
            market_condition = "Backwardation"
        else:
            market_condition = "Contango"

        # ── 동적 티커 (dra_sched에서 취득) ───────────────────────
        month_rows = dra_sched[
            (dra_sched.index.year == yr) & (dra_sched.index.month == mo)
        ]
        if month_rows.empty:
            continue

        roll_required           = bool(month_rows["roll_required"].iloc[0])
        dynamic_roll_out_ticker = month_rows["lead_ticker"].iloc[0]
        dynamic_roll_in_ticker  = month_rows["next_ticker"].iloc[0]

        if not roll_required:
            dynamic_roll_spread = 0.0
        else:
            dynamic_roll_spread = _avg_spread(
                dynamic_roll_out_ticker, dynamic_roll_in_ticker, roll_days
            )

        if pd.isna(dynamic_roll_spread) or pd.isna(static_roll_spread):
            spread_saving = float("nan")
        else:
            spread_saving = dynamic_roll_spread - static_roll_spread
        ...
        monthly_rows.append({
            ...
            "month":               mo,
            "market_condition":    market_condition,
            "static_roll_out":     static_roll_out,
            "static_roll_in":      static_roll_in,
            "static_roll_spread":  static_roll_spread,
            "dynamic_roll_out":    dynamic_roll_out_ticker,
            "dynamic_roll_in":     dynamic_roll_in_ticker,
            "roll_required":       roll_required,
            "dynamic_roll_spread": dynamic_roll_spread,
    ...
    if monthly_df.empty:
        return monthly_df, pd.DataFrame(columns=["항목", "정적", "동적"])

    N                      = len(monthly_df)
    dynamic_roll_months    = int(monthly_df["roll_required"].sum())
    dynamic_no_roll_months = N - dynamic_roll_months

    # ── 방식 1: 전체 월 평균 (roll_required=False → 0 포함) ──────
    static_avg_m1  = monthly_df["static_roll_spread"].mean()
    dynamic_avg_m1 = monthly_df["dynamic_roll_spread"].mean()
    saving_m1      = dynamic_avg_m1 - static_avg_m1

    # ── 방식 2: 롤 실행 월만 평균 ────────────────────────────────
    static_avg_m2  = static_avg_m1  # 정적은 방식1=방식2
    roll_only      = monthly_df.loc[monthly_df["roll_required"], "dynamic_roll_spread"]
    dynamic_avg_m2 = roll_only.mean() if dynamic_roll_months > 0 else float("nan")
    saving_m2      = dynamic_avg_m2 - static_avg_m2

    # ── Contango / Backwardation 분리 ────────────────────────────
    ct_df = monthly_df[monthly_df["market_condition"] == "Contango"]
    bw_df = monthly_df[monthly_df["market_condition"] == "Backwardation"]
    ct_count = len(ct_df)
    bw_count = len(bw_df)

    ct_static_avg  = ct_df["static_roll_spread"].mean()
    ct_dynamic_avg = ct_df["dynamic_roll_spread"].mean()
    bw_static_avg  = bw_df["static_roll_spread"].mean()
    bw_dynamic_avg = bw_df["dynamic_roll_spread"].mean()

    def _fmt(v, dec: int = 4) -> str:
        return "NaN" if pd.isna(v) else f"{v:.{dec}f}"

    pct = lambda n: f"{n} ({n / N * 100:.0f}%)"

    summary_rows = [
        {"항목": "분석 기간 총 월수",
         "정적": str(N),                "동적": str(N)},
        {"항목": "롤 실행 월수",
         "정적": str(N),                "동적": pct(dynamic_roll_months)},
        {"항목": "롤 미실행 월수 (스프레드 0)",
         "정적": "0",                   "동적": pct(dynamic_no_roll_months)},
        {"항목": "방식1 평균 롤 스프레드 ($/배럴)",
         "정적": _fmt(static_avg_m1),   "동적": _fmt(dynamic_avg_m1)},
        {"항목": "방식2 평균 롤 스프레드 ($/배럴)",
         "정적": _fmt(static_avg_m2),   "동적": _fmt(dynamic_avg_m2)},
        {"항목": "방식1 기준 절감 ($/배럴)",
         "정적": "-",                   "동적": _fmt(saving_m1)},
        {"항목": "방식2 기준 절감 ($/배럴)",
         "정적": "-",                   "동적": _fmt(saving_m2)},
        {"항목": "Contango 구간 월수",
         "정적": str(ct_count),         "동적": str(ct_count)},
        {"항목": "Backwardation 구간 월수",
         "정적": str(bw_count),         "동적": str(bw_count)},
        {"항목": "Contango 구간 평균 스프레드 (방식1, $/배럴)",
         "정적": _fmt(ct_static_avg),   "동적": _fmt(ct_dynamic_avg)},
        {"항목": "Backwardation 구간 평균 스프레드 (방식1, $/배럴)",
         "정적": _fmt(bw_static_avg),   "동적": _fmt(bw_dynamic_avg)},
    ]
...
# ============================================================
# 파트 16: 출력 파일 생성
# 기존: monthly_comparison / annual_summary / cumulative_summary /
#        dra_positions / comparison_chart
# 추가: pure_roll_yield / effective_roll_cost_daily /
#        effective_roll_cost_monthly / roll_cost_monthly /
#        roll_cost_summary
# ============================================================
...
def _save_all_outputs(
    ...
    output_dir: str,
    bdays: Optional[pd.DatetimeIndex] = None,
    prices_wide: Optional[pd.DataFrame] = None,
    expiration_map: Optional[dict] = None,
    static_result: Optional[pd.DataFrame] = None,
    dra_result: Optional[pd.DataFrame] = None,
    dra_sched: Optional[pd.DataFrame] = None,
):
    """
    monthly_comparison / annual_summary / cumulative_summary /
    dra_positions / comparison_chart 저장 (기존 유지).

    추가 저장 (bdays, prices_wide, expiration_map, static_result, dra_result 제공 시):
      pure_roll_yield.csv               — 통일된 순수 롤일드 비교
      effective_roll_cost_daily.csv     — 일별 유효 롤 비용
      effective_roll_cost_monthly.csv   — 월별 유효 롤 비용 + 연환산

    추가 저장 (bdays, prices_wide, dra_sched, dra_result 제공 시):
      roll_cost_monthly.csv             — 월별 순수 롤 비용(bps) 상세
      roll_cost_summary.csv             — 전체 기간 요약 (행 기반)
    """
    import matplotlib
    matplotlib.use("Agg")
    ...
    # 5. pure_roll_yield.csv
    PURE_IRY_KO = {
        "date":             "롤결정일",
        "year":             "연도",
        "month":            "월",
        "market_condition": "시장상황",
        "static_roll_out":  "정적_롤아웃_월물",
        "static_roll_in":   "정적_롤인_월물",
        "static_pure_iry":  "정적_순수IRY",
        "dynamic_roll_out": "동적_롤아웃_월물",
        "dynamic_roll_in":  "동적_롤인_월물",
        "dynamic_pure_iry": "동적_순수IRY",
        "roll_required":    "롤실행여부",
        "pure_iry_saving":  "순수IRY_절감액",
    }
    pure_df = monthly_df[
        ["date", "year", "month", "market_condition",
         "static_roll_out_contract", "static_roll_contract", "static_pure_iry",
         "dynamic_roll_out_contract", "dynamic_roll_contract", "dynamic_pure_iry",
         "roll_required", "pure_iry_saving"]
    ].rename(columns={
        "static_roll_out_contract":  "static_roll_out",
        "static_roll_contract":      "static_roll_in",
        "dynamic_roll_out_contract": "dynamic_roll_out",
        "dynamic_roll_contract":     "dynamic_roll_in",
    }).rename(columns=PURE_IRY_KO)
    pure_df.to_csv(out / "pure_roll_yield.csv", index=False)
    print(f"  pure_roll_yield.csv 저장 ({len(pure_df)}행)")

    # 6. comparison_chart.png
    _plot_charts(monthly_df, out / "comparison_chart.png")

    # 7. effective_roll_cost_daily.csv / effective_roll_cost_monthly.csv
    if (bdays is not None and prices_wide is not None
            and expiration_map is not None
            and static_result is not None
            and dra_result is not None):
        try:
            erc_daily, erc_monthly = calculate_effective_roll_cost(
                bdays, prices_wide, expiration_map, static_result, dra_result
            )
            erc_daily.to_csv(out / "effective_roll_cost_daily.csv")
            print(f"  effective_roll_cost_daily.csv 저장 ({len(erc_daily)}행)")
            erc_monthly.to_csv(out / "effective_roll_cost_monthly.csv", index=False)
            print(f"  effective_roll_cost_monthly.csv 저장 ({len(erc_monthly)}행, 마지막 행=연환산)")
        except Exception as e:
            warnings.warn(
                f"유효 롤 비용 계산 실패 — 파일 저장 건너뜀: {e}",
                UserWarning,
                stacklevel=2,
            )
    else:
        print("  [참고] effective_roll_cost 파일 저장 건너뜀 "
              "(bdays/prices_wide/expiration_map/static_result/dra_result 미제공)")

    # 8. roll_cost_monthly.csv / roll_cost_summary.csv
    if (bdays is not None and prices_wide is not None
            and dra_sched is not None and dra_result is not None):
        try:
            rc_monthly, rc_summary = calculate_roll_cost(
                dra_result, prices_wide, dra_sched, bdays
            )
            rc_monthly.to_csv(out / "roll_cost_monthly.csv", index=False)
            print(f"  roll_cost_monthly.csv 저장 ({len(rc_monthly)}행)")
            rc_summary.to_csv(out / "roll_cost_summary.csv", index=False)
            print(f"  roll_cost_summary.csv 저장 ({len(rc_summary)}행)")
        except Exception as e:
            warnings.warn(
                f"롤 비용 bps 계산 실패 — 파일 저장 건너뜀: {e}",
                UserWarning,
                stacklevel=2,
            )
    else:
        print("  [참고] roll_cost 파일 저장 건너뜀 "
              "(bdays/prices_wide/dra_sched/dra_result 미제공)")
...
def run_dra_index_calculation(
    ...
    output_dir : str, optional
        월별 비교 분석 결과 폴더 경로
    static_result : pd.DataFrame, optional
        run_index_calculation() 반환값.
        제공 시 effective_roll_cost_*.csv 파일이 함께 저장됨.
    ...
    # result 완성 후 _save_all_outputs() 호출 (dra_result=result 전달 가능)
    if output_dir:
        _save_all_outputs(
            monthly_results, positions, output_dir,
            bdays=bdays,
